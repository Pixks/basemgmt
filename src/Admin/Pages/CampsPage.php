...
final class CampsPage {
	...
	private function render_edit_form(?object $camp): void {
		...
		if ( $camp ) {
			...
			if ( ! empty($checklist_db) ) {
				...
				while ( count($checklist) < 10 ) {
					$checklist[] = [
						'label'       => '',
						'party'       => CampCaseRepository::CHECKLIST_PARTY_SHARED,
	...
	public function handle_save(): void {
		...
		CampCaseRepository::save_case($camp_id, [
			'process_stage'        => wp_unslash($_POST['process_stage'] ?? ''),
			'needs_attention'      => wp_unslash($_POST['needs_attention'] ?? ''),
			'risk_level'           => wp_unslash($_POST['risk_level'] ?? ''),
			'owner_user_id'        => wp_unslash($_POST['owner_user_id'] ?? ''),
			'next_action_due_date' => wp_unslash($_POST['next_action_due_date'] ?? ''),
			'notes'                => wp_unslash($_POST['case_notes'] ?? ''),
			'readiness_notes'      => wp_unslash($_POST['readiness_notes'] ?? ''),
			'stage_change_note'    => wp_unslash($_POST['stage_change_note'] ?? ''),
		]);

		CampCaseRepository::save_organizer($camp_id, [
			'organization_name'        => wp_unslash($_POST['organization_name'] ?? ''),
			'contact_person'           => wp_unslash($_POST['contact_person'] ?? ''),
			'contact_email'            => wp_unslash($_POST['contact_email'] ?? ''),
			'contact_phone'            => wp_unslash($_POST['contact_phone'] ?? ''),
			'billing_name'             => wp_unslash($_POST['billing_name'] ?? ''),
			'billing_tax_id'           => wp_unslash($_POST['billing_tax_id'] ?? ''),
			'billing_address'          => wp_unslash($_POST['billing_address'] ?? ''),
			'settlement_contact_name'  => wp_unslash($_POST['settlement_contact_name'] ?? ''),
			'settlement_contact_email' => wp_unslash($_POST['settlement_contact_email'] ?? ''),
			'settlement_contact_phone' => wp_unslash($_POST['settlement_contact_phone'] ?? ''),
			'notes'                    => wp_unslash($_POST['organizer_notes'] ?? ''),
		]);

		CampCaseRepository::save_prearrival($camp_id, [
			'arrival_date'          => wp_unslash($_POST['arrival_date'] ?? ''),
			'arrival_time'          => wp_unslash($_POST['arrival_time'] ?? ''),
			'departure_date'        => wp_unslash($_POST['departure_date'] ?? ''),
			'departure_time'        => wp_unslash($_POST['departure_time'] ?? ''),
			'declared_participants' => wp_unslash($_POST['declared_participants'] ?? '0'),
			'declared_staff'        => wp_unslash($_POST['declared_staff'] ?? '0'),
			'declared_support'      => wp_unslash($_POST['declared_support'] ?? '0'),
			'dietary_requirements'  => wp_unslash($_POST['dietary_requirements'] ?? ''),
			'allergens'             => wp_unslash($_POST['allergens'] ?? ''),
			'infrastructure_plan'   => wp_unslash($_POST['infrastructure_plan'] ?? ''),
			'additional_needs'      => wp_unslash($_POST['additional_needs'] ?? ''),
			'invoice_details'       => wp_unslash($_POST['invoice_details'] ?? ''),
			'authorized_contacts'   => wp_unslash($_POST['authorized_contacts'] ?? ''),
		]);
		...
		OperationLogger::log(
			...
			$camp_id,
			[
				'process_stage'   => sanitize_key(wp_unslash($_POST['process_stage'] ?? CampCaseRepository::STAGE_INQUIRY)),
				'needs_attention' => ! empty($_POST['needs_attention']),
				'risk_level'      => sanitize_key(wp_unslash($_POST['risk_level'] ?? CampCaseRepository::RISK_LOW)),
			]
		);
	...
	private function parse_checklist(mixed $raw): array {
		...
		$total_rows  = max(count($labels), count($parties), count($statuses), count($assigned), count($due_dates), count($comments));
		$items       = [];
