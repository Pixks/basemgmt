...
Panel powitalny z nazwą obozu, terminem trwania, statusem meldunku i ostatnimi liczbami. To samodzielna wersja sekcji „Panel”.
...
|---|---|---|---|---|
| Content | Text | `content.heading` | `⚜ Obóz harcerski` | Stały nadtytuł nad nazwą obozu. |
| Content | Text | `content.status_ok_label` | `✓ Meldunek złożony` | Etykieta statusu, gdy meldunek za dzień został wysłany. |
| Content | Text | `content.status_warn_label` | `⚠ Brak meldunku` | Etykieta statusu, gdy meldunku jeszcze nie ma. |
...
```twig
<div class="bm-el" x-data="bmCamp()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-loader" x-show="!camp">
    <span class="bm-spinner"></span>
    <span>Ładowanie danych obozu…</span>
  </div>
  ...
  <template x-if="camp">
    <div>
      <div class="bm-hero">
        <div class="bm-split">
          <div>
            <div style="font-size:.74rem;opacity:.75;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px;">{{ content.heading }}</div>
            <h2 x-text="camp.name" style="margin:0 0 6px;color:#fff;font-size:1.55rem;"></h2>
            <div style="font-size:.84rem;opacity:.85;">
              📅 <span x-text="camp.start_date"></span> – <span x-text="camp.end_date"></span>
            </div>
          </div>
          <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end;">
            <span x-show="submittedToday" class="bm-badge" style="background:rgba(255,255,255,.18);color:#fff;">{{ content.status_ok_label }}</span>
            <span x-show="!submittedToday" class="bm-badge" style="background:#D4A017;color:#fff;">{{ content.status_warn_label }}</span>
          </div>
        </div>
      </div>
      ...
      <template x-if="latestCount">
        <div class="bm-status-bar">
          <div class="bm-stat"><strong x-text="latestCount.participants ?? 0"></strong><span>Uczestnicy</span></div>
          <div class="bm-sep"></div>
          <div class="bm-stat"><strong x-text="latestCount.staff ?? 0"></strong><span>Kadra</span></div>
          <div class="bm-sep"></div>
          <div class="bm-stat"><strong x-text="latestCount.workers ?? 0"></strong><span>Pracownicy</span></div>
          <div class="bm-sep"></div>
          <div class="bm-stat"><strong x-text="latestCount.total ?? 0"></strong><span>Łącznie</span></div>
        </div>
      </template>
    </div>
  </template>
...
- Alpine: `window.bmCamp()` z `assets/js/bm-components-auth.js`.
- REST / helper: `bmApi.getCamp()` w `assets/js/bm-api.js`.
- Store: czyta `camp`, `latestCount`, `submittedToday` i `authenticated` z `Alpine.store('bm')` zasilanego przez `assets/js/bm-store.js`.
- PHP / konfiguracja: wymaga poprawnie działającego endpointu `GET panel/camp`; store ładuje dane również automatycznie po zalogowaniu.
...
- Element nie zapisuje danych — jest tylko widokiem store, więc może być osadzony wielokrotnie bez konfliktów.
- Jeżeli chcesz używać go bez ekranu logowania na tej samej stronie, zadbaj o wcześniejsze uruchomienie `Alpine.store('bm').init()` z `bm-store.js`.
