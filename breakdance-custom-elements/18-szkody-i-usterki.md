...
Element do zgłaszania szkód oraz przeglądu już zarejestrowanych usterek w teczce obozowej.

## 1. Nazwa elementu w Breakdance
`BM 18 / Szkody i usterki`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `🔧 Szkody i usterki` | Nagłówek sekcji szkód. |
| Content | Text | `content.add_label` | `+ Zgłoś szkodę` | Etykieta przycisku otwierającego formularz. |
| Content | Text | `content.save_label` | `Zgłoś szkodę` | Etykieta przycisku zapisującego szkodę. |
| Content | Textarea | `content.empty_text` | `Brak zgłoszonych szkód.` | Komunikat przy pustej liście szkód. |
...
```twig
<div class="bm-el" x-data="bmDamages()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    ...
    <div class="bm-card-body">
      <div class="bm-loader" x-show="loading"><span class="bm-spinner"></span><span>Ładowanie szkód…</span></div>
      <div class="bm-alert bm-alert--error" x-show="error && !showForm" x-text="error"></div>
      ...
      <template x-if="showForm">
        <div class="bm-note" style="background:#EBF5EE;border:1px solid #A8D5B5;margin-bottom:16px;">
          <div style="font-weight:700;margin-bottom:12px;color:var(--bm-primary);">Nowe zgłoszenie szkody</div>
          <div class="bm-alert bm-alert--error" x-show="error" x-text="error"></div>
          <div class="bm-field">
            <label class="bm-label">Nazwa szkody *</label>
            <input class="bm-input" x-model="form.name" placeholder="np. Uszkodzona umywalka">
          </div>
          <div class="bm-field">
            <label class="bm-label">Opis</label>
            <textarea class="bm-textarea" x-model="form.description" placeholder="Dokładny opis szkody…"></textarea>
          </div>
          <div class="bm-field">
            <label class="bm-label">Szacunkowy koszt naprawy (zł)</label>
            <input class="bm-input" type="number" min="0" step="0.01" x-model="form.cost" placeholder="0.00" style="max-width:180px;">
          </div>
      ...
      <div class="bm-empty" x-show="!loading && !showForm && damages.length === 0">{{ content.empty_text }}</div>

      <template x-if="!loading && damages.length > 0">
        <div>
          <template x-for="d in damages" :key="d.id">
            <div class="bm-list-item bm-soft">
              <div class="bm-split">
                <div>
                  <div style="font-weight:700;" x-text="d.name"></div>
                  <div x-show="d.description" class="bm-meta" x-text="d.description" style="margin-top:3px;"></div>
                </div>
                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;">
                  <span class="bm-badge" :class="statusClass(d.status)" x-text="statusLabel(d.status)"></span>
                  <span x-show="d.cost > 0" class="bm-meta">~<span x-text="d.cost.toFixed(2)"></span> zł</span>
                </div>
              </div>
              <div class="bm-meta" style="margin-top:6px;" x-text="'Zgłoszono: ' + d.created_at"></div>
            </div>
          </template>
        </div>
      </template>
...
- Alpine: `window.bmDamages()` z `assets/js/bm-components-social.js`.
- REST / helper: `bmApi.getFolderDamages()`, `bmApi.reportDamage(payload)` w `assets/js/bm-api.js`.
- PHP / konfiguracja: dodawanie szkody wymaga `bmConfig.panelNonce`; komponent normalizuje koszt przez `parseFloat(this.form.cost) || 0`.
...
- Statusy szkód są mapowane w JS do klas badge zgodnych z paletą ZHP.
- To niezależny blok teczki — można osadzić go samodzielnie np. na stronie tylko dla logistyki obozu.
