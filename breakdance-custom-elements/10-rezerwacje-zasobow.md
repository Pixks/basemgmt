# 10. Rezerwacje zasobów

Kompletny element do podglądu zasobów, założenia rezerwacji oraz listy własnych zgłoszeń z możliwością anulowania.

## 1. Nazwa elementu w Breakdance
`BM 10 / Rezerwacje zasobów`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `🏕 Rezerwacje zasobów` | Nagłówek karty rezerwacji. |
| Content | Text | `content.reserve_label` | `Zarezerwuj →` | Etykieta przycisku rozpoczęcia rezerwacji. |
| Content | Textarea | `content.empty_text` | `Brak dostępnych zasobów.` | Komunikat przy pustej liście zasobów. |
...
```twig
<div class="bm-el" x-data="bmReservations()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header"><h3>{{ content.heading }}</h3></div>

    <div class="bm-card-body">
      <div class="bm-loader" x-show="loading"><span class="bm-spinner"></span><span>Ładowanie rezerwacji…</span></div>
      <div class="bm-alert bm-alert--success" x-show="success" x-text="success"></div>

      <template x-if="!selectedResource">
        <div>
          <div class="bm-grid-auto">
            <template x-for="res in resources" :key="res.id">
              <div class="bm-resource">
                <div class="bm-split" style="margin-bottom:7px;">
                  <strong x-text="res.name"></strong>
                  <span class="bm-badge bm-badge--green" x-text="res.type"></span>
                </div>
                <p class="bm-meta" x-text="'🕒 ' + res.available_from + ' – ' + res.available_to" style="margin-bottom:4px;"></p>
                <p x-show="res.rules" x-text="res.rules" class="bm-meta" style="margin-bottom:10px;font-style:italic;"></p>
                <button class="bm-btn bm-btn--primary bm-btn--sm" @click="openForm(res)">{{ content.reserve_label }}</button>
              </div>
            </template>
          </div>
          <div class="bm-empty" x-show="!loading && !resources.length">{{ content.empty_text }}</div>
        </div>
      </template>

      <template x-if="selectedResource">
        <div class="bm-note" style="background:#EBF5EE;border:1px solid #A8D5B5;">
          <h4 style="margin:0 0 14px;color:var(--bm-primary);">Rezerwacja: <span x-text="selectedResource.name"></span></h4>
          <form @submit.prevent="submitReservation()">
            <div class="bm-field">
              <label class="bm-label">Data *</label>
              <input type="date" class="bm-input" x-model="form.res_date" @change="loadSlots()" required>
            </div>

            <template x-if="takenSlots.length">
              <div class="bm-meta" style="margin-bottom:10px;">
                <strong>Zajęte:</strong>
                <template x-for="slot in takenSlots" :key="slot.start_time">
                  <span class="bm-badge bm-badge--red" style="margin-left:4px;" x-text="slot.start_time.slice(0,5) + '–' + slot.end_time.slice(0,5)"></span>
                </template>
              </div>
            </template>

            <template x-if="blockWindows.length">
              <div class="bm-alert bm-alert--warn" style="flex-direction:column;align-items:flex-start;">
            ...
            <div class="bm-grid-2">
              <div class="bm-field">
                <label class="bm-label">Od *</label>
                <input type="time" class="bm-input" x-model="form.start_time" required>
              </div>
              <div class="bm-field">
                <label class="bm-label">Do *</label>
                <input type="time" class="bm-input" x-model="form.end_time" required>
              </div>
            </div>

            <div class="bm-field">
              <label class="bm-label">Cel rezerwacji *</label>
              <input type="text" class="bm-input" x-model="form.purpose" required placeholder="np. Mecz piłkarski">
            </div>

            <div class="bm-alert bm-alert--error" x-show="formError" x-text="formError"></div>

            <div class="bm-grid-2">
              <button type="submit" class="bm-btn bm-btn--primary" :disabled="loading" x-text="loading ? 'Wysyłanie…' : 'Zarezerwuj'"></button>
              <button type="button" class="bm-btn bm-btn--ghost" @click="selectedResource = null">Anuluj</button>
            </div>
          </form>
        </div>
      </template>
      ...
      <details style="margin-top:18px;">
        <summary style="cursor:pointer;font-weight:700;color:var(--bm-primary);">📋 Moje rezerwacje</summary>
        <div class="bm-table-wrap" style="margin-top:10px;">
            ...
            <tbody>
              <template x-for="r in myReservations" :key="r.id">
                <tr>
                  ...
                  <td><span class="bm-badge" :class="r.status==='approved' ? 'bm-badge--green' : r.status==='pending' ? 'bm-badge--gold' : 'bm-badge--red'" x-text="r.status_label || r.status"></span></td>
                  <td><button x-show="r.status==='pending'" class="bm-btn bm-btn--danger bm-btn--sm" @click="cancel(r.id)">Anuluj</button></td>
                </tr>
              </template>
            </tbody>
          </table>
...
- Alpine: `window.bmReservations()` z `assets/js/bm-components-content.js`.
- REST / helper: `bmApi.getResources()`, `bmApi.getReservationSlots(resourceId, date)`, `bmApi.getMyReservations(status)`, `bmApi.createReservation(payload)`, `bmApi.cancelReservation(id)` w `assets/js/bm-api.js`.
- Zdarzenia przeglądarki: `bm:reservationCreated`, `bm:reservationCancelled`.
- PHP / konfiguracja: tworzenie rezerwacji wymaga `bmConfig.panelNonce`; anulowanie korzysta z natywnego `confirm()` w komponencie.
...
- Element wymaga `assets/js/bm-store.js`, `assets/js/bm-api.js` oraz `assets/js/bm-components-content.js`.
...
- Widok formularza i katalog zasobów korzystają z jednego stanu `selectedResource`; nie rozbijaj tego na dwa osobne elementy bez przebudowy JS.
- `blockWindows` i `takenSlots` przychodzą bezpośrednio z API, więc można je wykorzystać także do dodatkowych ostrzeżeń w Breakdance.
