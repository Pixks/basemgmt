...
Karta pogodowa z bieżącą temperaturą, prognozą i ostrzeżeniami. To wyodrębniona sekcja pogodowa z widoku głównego panelu.

## 1. Nazwa elementu w Breakdance
`BM 05 / Pogoda i alerty`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `🌤 Pogoda` | Nagłówek karty pogodowej. |
| Content | Textarea | `content.unconfigured_text` | `Lokalizacja nie skonfigurowana.` | Komunikat przy braku danych lokalizacji. |
| Content | Textarea | `content.empty_text` | `Brak danych pogodowych.` | Tekst awaryjny, gdy API nie zwróci obiektu `weather`. |
...
```twig
<div class="bm-el" x-data="bmWeather()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header"><h3>{{ content.heading }}</h3></div>

    <div class="bm-card-body">
      <div class="bm-loader" x-show="loading">
        <span class="bm-spinner"></span>
        <span>Ładowanie pogody…</span>
      </div>

      <div class="bm-alert bm-alert--warn" x-show="!configured && !loading">{{ content.unconfigured_text }}</div>
      <div class="bm-alert bm-alert--error" x-show="error" x-text="error"></div>

      <template x-if="current">
        <div class="bm-weather-card">
          <div style="font-size:3rem;line-height:1;" x-text="current.icon"></div>
          <div>
            <div class="bm-weather-temp" x-text="current.temperature + '°C'"></div>
            <div style="font-weight:700;" x-text="current.label"></div>
            <div class="bm-meta" style="margin-top:4px;">
      ...
      <div class="bm-empty" x-show="!loading && !current && !error">{{ content.empty_text }}</div>

      <div class="bm-forecast" x-show="forecast.length">
        <template x-for="day in forecast" :key="day.date">
          <div class="bm-forecast-day">
            <div class="bm-meta" x-text="day.date.slice(5)"></div>
            <div style="font-size:1.35rem;" x-text="day.icon"></div>
            <div style="font-weight:700;color:var(--bm-primary);" x-text="day.temp_max + '°'"></div>
            <div class="bm-meta" x-text="day.temp_min + '°'"></div>
          </div>
        </template>
      </div>

      <template x-for="alert in alerts" :key="alert.id">
        <div :class="alert.is_urgent ? 'bm-alert bm-alert--error' : 'bm-alert bm-alert--warn'" style="margin-top:10px;flex-direction:column;align-items:flex-start;">
          <strong x-text="(alert.is_urgent ? '🚨 ' : '⚠ ') + alert.title"></strong>
...
%%SELECTOR%% .bm-weather-temp { font-size: 2.4rem; font-weight: 700; color: var(--bm-primary); line-height: 1; }
```

## 5. JavaScript / Alpine.js / PHP / integracje

- Alpine: `window.bmWeather()` z `assets/js/bm-components-content.js`.
- REST / helper: `bmApi.getWeather()` w `assets/js/bm-api.js`.
- PHP / konfiguracja: wymaga działającego `GET panel/weather`; backend musi zwrócić pola `weather`, `alerts`, `configured` i `location` zgodnie z komponentem.
...
- Alerty pogodowe są renderowane wprost z odpowiedzi API; nie potrzebujesz dodatkowego mapowania w Twig.
- Jeżeli chcesz mocniej wyeksponować alarmy IMGW, dołóż w Breakdance condition lub zewnętrzne powiadomienie nasłuchujące store / eventów po załadowaniu danych.
