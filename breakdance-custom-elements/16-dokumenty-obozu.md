...
Karta listująca dokumenty przypisane do konkretnego obozu wraz ze statusem, blokadą i informacją o podpisie.

## 1. Nazwa elementu w Breakdance
`BM 16 / Dokumenty obozu`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `📋 Dokumenty obozu` | Nagłówek sekcji dokumentów obozu. |
| Content | Text | `content.download_label` | `⬇ Pobierz` | Etykieta przycisku pobierania. |
| Content | Textarea | `content.empty_text` | `Brak dokumentów przypisanych do obozu.` | Komunikat przy pustej liście. |
...
```twig
<div class="bm-el" x-data="bmCampDocuments()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header"><h3>{{ content.heading }}</h3></div>
    <div class="bm-card-body">
      <div class="bm-loader" x-show="loading"><span class="bm-spinner"></span><span>Ładowanie dokumentów obozu…</span></div>
          ...
          <template x-for="doc in documents" :key="doc.id">
            <div class="bm-doc-row">
              <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0;">
                <span style="font-size:1.3rem;" x-text="docIcon(doc.document_type)"></span>
                <div style="min-width:0;">
                  <div style="font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" x-text="doc.title"></div>
                  <div class="bm-pills" style="margin-top:4px;">
                    <span class="bm-badge" :class="statusClass(doc.status)" x-text="statusLabel(doc.status)"></span>
                    <span x-show="doc.signed_at" class="bm-badge bm-badge--green">✓ podpisany</span>
                    <span x-show="doc.locked" class="bm-badge bm-badge--gray">🔒 zablokowany</span>
                  </div>
                </div>
              </div>
...
- Alpine: `window.bmCampDocuments()` z `assets/js/bm-components-social.js`.
- REST / helper: `bmApi.getCampDocuments()` w `assets/js/bm-api.js`.
- PHP / konfiguracja: element oczekuje `camp_documents` z polami `status`, `signed_at`, `locked`, `document_type` i `file_url`.
...
- Mapowanie statusów odbywa się w `statusLabel()` i `statusClass()` po stronie JS; dzięki temu Twig pozostaje prosty.
- Ten blok odpowiada drugiej sekcji „Teczka” w panelu pełnym i nie wymaga osobnej nawigacji.
