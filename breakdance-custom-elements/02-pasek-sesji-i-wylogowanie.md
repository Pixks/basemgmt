# 02. Pasek sesji i wylogowanie

Belka górna z aktualnie zalogowanym użytkownikiem, nazwą obozu i przyciskiem wylogowania. To rozdzielona wersja topbara z pełnego panelu.

## 1. Nazwa elementu w Breakdance
`BM 02 / Pasek sesji i wylogowanie`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.logout_label` | `Wyloguj się ↩` | Etykieta przycisku wylogowania. |
| Content | Text | `content.fallback_name` | `Zalogowany użytkownik` | Tekst zastępczy, gdy `displayName` nie jest jeszcze w store. |
| Content | Text | `content.fallback_camp` | `Obóz niezaładowany` | Tekst zastępczy dla nazwy obozu. |
...
```twig
<div class="bm-el" x-data="bmLogout()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card bm-userbar">
    <div class="bm-userbar__identity">
      <div class="bm-avatar">⚜</div>
      <div>
        <div class="bm-userbar__name" x-text="$store.bm.displayName || '{{ content.fallback_name }}'"></div>
        <div class="bm-meta" x-text="$store.bm.camp?.name || '{{ content.fallback_camp }}'"></div>
      </div>
    </div>
    <button class="bm-btn bm-btn--ghost bm-btn--sm" @click="logout()">{{ content.logout_label }}</button>
  </div>
</div>
```
...
%%SELECTOR%% .bm-userbar__identity { display: flex; align-items: center; gap: 12px; }
%%SELECTOR%% .bm-avatar {
  width: 42px;
  height: 42px;
  border-radius: 50%;
...
%%SELECTOR%% .bm-userbar__name { font-weight: 700; font-size: .95rem; }
```

## 5. JavaScript / Alpine.js / PHP / integracje

- Alpine: `window.bmLogout()` z `assets/js/bm-components-auth.js`.
- REST / helper: `bmApi.logout()` w `assets/js/bm-api.js`.
- Store: po wylogowaniu czyści `Alpine.store('bm')` metodą `clearSession()` z `assets/js/bm-store.js`.
- Zdarzenia przeglądarki: `bm:logout`.
- PHP / konfiguracja: nie wymaga dodatkowego PHP poza poprawnym `bmConfig.wpNonce`; komponent po akcji robi `window.location.reload()`.
...
- To element stricte sesyjny: najlepiej osadzić go na górze szablonu strony panelowej.
- Jeśli nie chcesz automatycznego przeładowania po wylogowaniu, zmodyfikuj komponent źródłowy w JS — nie w samym Twig elementu.
