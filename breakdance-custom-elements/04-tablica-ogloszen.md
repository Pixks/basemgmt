...
Pełna lista aktywnych ogłoszeń dla kadry: pilne komunikaty na górze, standardowe poniżej. Element jest niezależny od głównej zakładki panelu.

## 1. Nazwa elementu w Breakdance
`BM 04 / Tablica ogłoszeń`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `📢 Ogłoszenia` | Tytuł karty z ogłoszeniami. |
| Content | Text | `content.refresh_label` | `↻ Odśwież` | Etykieta przycisku odświeżania. |
| Content | Textarea | `content.empty_text` | `Brak aktywnych ogłoszeń.` | Komunikat przy pustej liście. |
...
<div class="bm-el" x-data="bmAnnouncements()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header">
      ...
      <button class="bm-btn bm-btn--light bm-btn--sm" @click="refresh()">{{ content.refresh_label }}</button>
    </div>
    ...
    <div class="bm-card-body">
      <template x-for="ann in active.filter(a => a.is_urgent)" :key="ann.id">
        <div class="bm-alert bm-alert--error" style="flex-direction:column;align-items:flex-start;">
          <div class="bm-split" style="width:100%;">
            <strong>🚨 <span x-text="ann.title"></span></strong>
            <span class="bm-badge bm-badge--red">Pilne</span>
          </div>
          <div x-html="ann.content"></div>
          <div class="bm-meta" style="display:flex;gap:10px;flex-wrap:wrap;">
            <span>Ważne do: <strong x-text="ann.valid_until || '—'"></strong></span>
            <a x-show="ann.attachment_url" :href="ann.attachment_url" target="_blank">📎 Załącznik</a>
          </div>
        </div>
      </template>
      ...
      <template x-for="ann in active.filter(a => !a.is_urgent)" :key="ann.id">
        <div class="bm-list-item">
          <div class="bm-split">
            ...
            <span class="bm-badge bm-badge--green">Aktywne</span>
          </div>
          <div x-html="ann.content" style="margin-top:7px;color:#4B5A67;"></div>
          <div class="bm-meta" style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;margin-top:7px;">
...
- Alpine: `window.bmAnnouncements()` z `assets/js/bm-components-auth.js`.
- REST / helper: `bmApi.getAnnouncements()` w `assets/js/bm-api.js`.
- Store: czyta `announcements.active`, `announcements.archived` i `announcements.own` z `Alpine.store('bm')`; metoda `refresh()` wywołuje `loadAnnouncements()` ze store.
- PHP / konfiguracja: dane dostarcza `GET panel/announcements`; plugin ładuje pierwsze wyniki automatycznie po inicjalizacji store.
...
- W źródłowym panelu widok pełny pokazuje tylko `active`; jeśli chcesz dodać archiwum, możesz dołożyć drugi blok na `archived` bez zmiany komponentu JS.
- Zawartość `ann.content` jest renderowana przez `x-html`, więc backend powinien nadal filtrować HTML po swojej stronie.
