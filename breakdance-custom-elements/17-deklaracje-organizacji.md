# 17. Deklaracje organizacji

Lista deklaracji przesłanych do obozu z możliwością pobrania i zatwierdzenia dokumentu przez kadrę obozową.

## 1. Nazwa elementu w Breakdance
`BM 17 / Deklaracje organizacji`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `📑 Deklaracje organizacji` | Nagłówek sekcji deklaracji. |
| Content | Text | `content.download_label` | `⬇ Pobierz` | Etykieta przycisku pobierania dokumentu. |
| Content | Text | `content.approve_label` | `✓ Zatwierdź` | Etykieta przycisku zatwierdzania. |
| Content | Textarea | `content.empty_text` | `Brak deklaracji przesłanych do obozu.` | Komunikat przy pustej liście. |
...
```twig
<div class="bm-el" x-data="bmDeclDocs()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    ...
    <div class="bm-card-body">
      ...
      <div class="bm-empty" x-show="!loading && !error && docs.length === 0">{{ content.empty_text }}</div>
            ...
            <div class="bm-list-item bm-soft">
              <div class="bm-split">
                <div style="flex:1;min-width:0;">
                  <div style="font-weight:700;margin-bottom:4px;" x-text="doc.title"></div>
                  <div class="bm-pills">
                    <span class="bm-badge" :class="statusClass(doc)" x-text="statusLabel(doc)"></span>
                    <span x-show="doc.camp_approved_at" style="font-size:.74rem;color:var(--bm-primary);" x-text="'✓ Zatwierdzone: ' + doc.camp_approved_at?.slice(0,10)"></span>
                  </div>
                </div>
                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                  ...
                  <button x-show="!doc.camp_approved_at" class="bm-btn bm-btn--primary bm-btn--sm" :disabled="approving === doc.id" @click="approve(doc.id)">
                    <span x-show="approving !== doc.id">{{ content.approve_label }}</span>
                    <span x-show="approving === doc.id" class="bm-spinner" style="width:12px;height:12px;border-width:2px;"></span>
                  </button>
                </div>
              </div>
              <div class="bm-meta" style="margin-top:6px;" x-text="'Przesłano: ' + doc.created_at?.slice(0,10)"></div>
            </div>
          </template>
        </div>
      </template>
...
- Alpine: `window.bmDeclDocs()` z `assets/js/bm-components-social.js`.
- REST / helper: `bmApi.getDeclDocs()`, `bmApi.approveDeclDoc(id, nonce)` w `assets/js/bm-api.js`.
- PHP / konfiguracja: zatwierdzenie używa `bmConfig.panelNonce` i wbudowanego `confirm()`; backend powinien pilnować nieodwracalności akcji.
...
- Komponent ma własny stan `approving`, więc spinner zatwierdzania działa per rekord bez dodatkowych helperów.
- Status jest liczony na podstawie kombinacji pól `camp_approved_at`, `approved_at` i `signed_at` zwróconych przez API.
