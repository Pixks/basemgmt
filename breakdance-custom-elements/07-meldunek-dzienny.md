...
Kompletny element do zapisu roboczego i wysyłki meldunku dziennego wraz z krótką historią ostatnich wpisów.

## 1. Nazwa elementu w Breakdance
`BM 07 / Meldunek dzienny`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `📋 Meldunek dzienny` | Nagłówek karty meldunku. |
| Content | Text | `content.save_label` | `💾 Zapisz roboczo` | Etykieta przycisku zapisu roboczego. |
| Content | Text | `content.submit_label` | `✓ Wyślij meldunek` | Etykieta przycisku wysyłki meldunku. |
...
```twig
<div class="bm-el" x-data="bmReports()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header">
      <h3>{{ content.heading }}</h3>
      <span class="bm-badge" :class="today?.status==='submitted' ? 'bm-badge--green' : today?.status==='draft' ? 'bm-badge--gold' : 'bm-badge--gray'" x-text="statusLabel || 'Brak'"></span>
    </div>

    <div class="bm-card-body">
      <div class="bm-alert bm-alert--success" x-show="isSubmitted">✓ Meldunek wysłany. Nie można go już modyfikować.</div>

      <fieldset :disabled="isSubmitted" style="border:none;padding:0;margin:0;">
        <div class="bm-grid-3">
          <div class="bm-field">
            <label class="bm-label">Uczestnicy</label>
            <input type="number" class="bm-input" x-model.number="form.participants" min="0" style="text-align:center;font-size:1.35rem;font-weight:700;">
          </div>
          <div class="bm-field">
            <label class="bm-label">Kadra</label>
            <input type="number" class="bm-input" x-model.number="form.staff" min="0" style="text-align:center;font-size:1.35rem;font-weight:700;">
          </div>
          <div class="bm-field">
            <label class="bm-label">Pracownicy</label>
            <input type="number" class="bm-input" x-model.number="form.workers" min="0" style="text-align:center;font-size:1.35rem;font-weight:700;">
          </div>
        </div>

        <div class="bm-note" style="margin-bottom:12px;text-align:center;background:#EBF5EE;border:1px solid #A8D5B5;">
          <div class="bm-label" style="margin-bottom:2px;">Łącznie</div>
          <div style="font-size:2rem;font-weight:700;color:var(--bm-primary);" x-text="total"></div>
        </div>

        <div class="bm-field">
          <label class="bm-label">Uwagi</label>
          <textarea class="bm-textarea" x-model="form.notes" rows="2" placeholder="opcjonalnie…"></textarea>
        </div>
      </fieldset>
      ...
      <div class="bm-grid-2" x-show="!isSubmitted">
        <button class="bm-btn bm-btn--ghost" @click="saveDraft()" :disabled="loading" x-text="loading ? 'Zapisywanie…' : '{{ content.save_label }}'"></button>
        <button class="bm-btn bm-btn--primary" @click="submit()" :disabled="loading" x-text="loading ? 'Wysyłanie…' : '{{ content.submit_label }}'"></button>
      </div>

      <details style="margin-top:18px;">
        <summary style="cursor:pointer;font-weight:700;color:var(--bm-primary);">📊 Historia meldunków</summary>
        <div class="bm-table-wrap" style="margin-top:10px;">
          <table class="bm-table">
            <thead>
              <tr><th>Data</th><th>Ucz.</th><th>Kadra</th><th>Prac.</th><th>Status</th></tr>
            </thead>
            <tbody>
              <template x-for="r in history.slice(0, 7)" :key="r.id">
                <tr>
                  <td x-text="r.count_date"></td>
                  <td x-text="r.participants" style="text-align:center;"></td>
                  <td x-text="r.staff" style="text-align:center;"></td>
                  <td x-text="r.workers" style="text-align:center;"></td>
                  <td style="text-align:center;">
                    <span class="bm-badge" :class="r.status==='submitted' ? 'bm-badge--green' : r.status==='draft' ? 'bm-badge--gold' : 'bm-badge--gray'" x-text="r.status==='submitted' ? 'wysłany' : r.status==='draft' ? 'roboczy' : '—'"></span>
                  </td>
                </tr>
              </template>
            </tbody>
...
- Alpine: `window.bmReports()` z `assets/js/bm-components-content.js`.
- REST / helper: `bmApi.getReportToday()`, `bmApi.saveReportDraft(payload)`, `bmApi.submitReport(payload)`, `bmApi.getReportHistory(limit)` w `assets/js/bm-api.js`.
- Zdarzenia przeglądarki: `bm:reportSaved`, `bm:reportSubmitted`.
- PHP / konfiguracja: wymaga `bmConfig.panelNonce`; komponent pokazuje natywne `confirm()` przed ostateczną wysyłką.
...
- Źródłowy panel używa `bmReports()` zamiast starego `bmDailyCount()`; nie mieszaj tych dwóch elementów na jednej stronie.
- Po wysłaniu meldunku komponent blokuje formularz przez `isSubmitted`, więc nie trzeba dokładać osobnego read-only switcha.
