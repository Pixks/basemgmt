# 20. Deklaracja pobytu

Końcowa sekcja teczki obozowej: podgląd deklaracji głównej oraz edycja / dodawanie wpisów dziennych pobytu.

## 1. Nazwa elementu w Breakdance
`BM 20 / Deklaracja pobytu`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `📋 Deklaracja pobytu` | Nagłówek sekcji deklaracji pobytu. |
| Content | Text | `content.add_day_label` | `+ Dodaj dzień` | Etykieta przycisku dodawania nowego dnia. |
| Content | Text | `content.save_label` | `Zapisz` | Etykieta przycisku zapisu dnia. |
| Content | Textarea | `content.empty_text` | `Brak wpisów dziennych. Użyj przycisku „+ Dodaj dzień”, aby dodać pierwszy.` | Komunikat przy pustej tabeli dni. |
...
```twig
<div class="bm-el" x-data="bmDeclaration()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header"><h3>{{ content.heading }}</h3></div>
    <div class="bm-card-body">
      <div class="bm-loader" x-show="loading"><span class="bm-spinner"></span><span>Ładowanie deklaracji…</span></div>
      <div class="bm-alert bm-alert--error" x-show="error && !editing" x-text="error"></div>

      <template x-if="!loading && declaration">
        <div class="bm-note" style="margin-bottom:16px;">
          <div class="bm-grid-auto" style="grid-template-columns:repeat(auto-fit,minmax(130px,1fr));">
            <div><div class="bm-label">Zad. osoby</div><div style="font-size:1.1rem;font-weight:700;" x-text="declaration.declared_persons"></div></div>
            <div><div class="bm-label">Zad. diety</div><div style="font-size:1.1rem;font-weight:700;color:var(--bm-primary-hover);" x-text="declaration.declared_diets || '—'"></div></div>
            <div><div class="bm-label">Przyjazd</div><div x-text="declaration.arrival_time || '—'"></div></div>
            <div><div class="bm-label">Odjazd</div><div x-text="declaration.departure_time || '—'"></div></div>
            <div>
              <div class="bm-label">Status</div>
              <span class="bm-badge" :class="declaration.signed_at ? 'bm-badge--green' : declaration.submitted_at ? 'bm-badge--gold' : 'bm-badge--gray'" x-text="declaration.signed_at ? 'Podpisana' : declaration.submitted_at ? 'Wysłana' : 'W przygotowaniu'"></span>
            </div>
          </div>
          <div x-show="declaration.notes" style="margin-top:12px;background:#FEF8E7;padding:8px 12px;border-radius:6px;color:#7A5800;">📝 <span x-text="declaration.notes"></span></div>
        </div>
      </template>

      <template x-if="!loading && !declaration">
        <div class="bm-alert bm-alert--info">Deklaracja obozu nie została jeszcze zainicjowana przez organizatora.</div>
      </template>

      <div class="bm-toolbar">
        <div style="font-weight:700;color:#4B5A67;">Deklaracja dzienna</div>
        <button class="bm-btn bm-btn--ghost bm-btn--sm" @click="newDay()" x-show="!editing">{{ content.add_day_label }}</button>
      </div>

      <template x-if="editing">
        <div class="bm-note" style="background:#EBF5EE;border:1px solid #A8D5B5;margin-bottom:16px;">
          <div style="font-weight:700;margin-bottom:12px;color:var(--bm-primary);" x-text="editing.declaration_date ? 'Edytuj dzień: ' + editing.declaration_date : 'Nowy dzień deklaracji'"></div>
          <div class="bm-alert bm-alert--error" x-show="error" x-text="error"></div>
          <div class="bm-grid-2">
            <div class="bm-field" style="grid-column:1/-1;">
              <label class="bm-label">Data *</label>
              <input class="bm-input" type="date" x-model="editing.declaration_date">
            </div>
            <div class="bm-field">
              <label class="bm-label">Liczba osób</label>
              <input class="bm-input" type="number" min="0" x-model="editing.declared_persons">
            </div>
            <div class="bm-field">
              <label class="bm-label">Godzina przyjazdu</label>
              <input class="bm-input" type="time" x-model="editing.arrival_time">
            </div>
            <div class="bm-field">
              <label class="bm-label">Godzina odjazdu</label>
              <input class="bm-input" type="time" x-model="editing.departure_time">
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button class="bm-btn bm-btn--primary" @click="saveDay()" :disabled="saving">
              <span x-show="!saving">{{ content.save_label }}</span>
              <span x-show="saving" class="bm-spinner" style="width:14px;height:14px;border-width:2px;"></span>
            </button>
            <button class="bm-btn bm-btn--ghost" @click="cancelEdit()">Anuluj</button>
          </div>
        </div>
      </template>

      <div class="bm-empty" x-show="!loading && !editing && days.length === 0">{{ content.empty_text }}</div>

      <template x-if="!loading && days.length > 0">
        <div class="bm-table-wrap">
          <table class="bm-table">
            <thead><tr><th>Data</th><th>Osoby</th><th>Przyjazd</th><th>Odjazd</th><th></th></tr></thead>
            <tbody>
              <template x-for="day in days" :key="day.id">
                <tr>
                  <td x-text="fmtDate(day.declaration_date)"></td>
                  <td style="text-align:right;font-weight:700;" x-text="day.declared_persons"></td>
                  <td style="text-align:center;" x-text="day.arrival_time || '—'"></td>
                  <td style="text-align:center;" x-text="day.departure_time || '—'"></td>
                  <td><button class="bm-btn bm-btn--ghost bm-btn--sm" @click="editDay(day)">✏️</button></td>
                </tr>
              </template>
            </tbody>
          </table>
...
- Alpine: `window.bmDeclaration()` z `assets/js/bm-components-social.js`.
- REST / helper: `bmApi.getFolderDeclaration()`, `bmApi.saveDeclarationDay(payload)` w `assets/js/bm-api.js`.
- PHP / konfiguracja: zapis dnia wymaga `bmConfig.panelNonce`; nagłówek deklaracji jest tylko do odczytu i pochodzi z odpowiedzi `GET panel/folder/declaration`.
...
- Źródłowy panel nie przewiduje edycji danych nagłówkowych deklaracji w Breakdance — tylko odczyt i edycję dni pobytu.
- Formatowanie dat jest obsługiwane helperem `fmtDate()` w JS; pozostaw je w Twig jako zwykłe `x-text`.
