...
Element do rejestrowania wydanego sprzętu, kontrolowania zwrotów i podglądu pozycji oczekujących na rozliczenie.
...
|---|---|---|---|---|
| Content | Text | `content.heading` | `📦 Sprzęt obozu` | Nagłówek sekcji sprzętu. |
| Content | Text | `content.add_label` | `+ Wydaj sprzęt` | Etykieta przycisku otwierającego formularz nowej pozycji. |
| Content | Text | `content.save_label` | `Zapisz` | Etykieta przycisku zapisu pozycji. |
| Content | Textarea | `content.empty_text` | `Brak zapisanego sprzętu.` | Komunikat przy pustej tabeli. |
...
```twig
<div class="bm-el" x-data="bmEquipment()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header">
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        <h3>{{ content.heading }}</h3>
        <span x-show="!loading && totalOutstanding > 0" class="bm-badge bm-badge--gold" x-text="totalOutstanding + ' szt. do zwrotu'"></span>
      </div>
      <button class="bm-btn bm-btn--light bm-btn--sm" @click="openForm()" x-show="!showForm">{{ content.add_label }}</button>
    </div>

    <div class="bm-card-body">
      <div class="bm-loader" x-show="loading"><span class="bm-spinner"></span><span>Ładowanie sprzętu…</span></div>
      ...
      <template x-if="showForm">
        <div class="bm-note" style="background:#EBF5EE;border:1px solid #A8D5B5;margin-bottom:16px;">
          <div style="font-weight:700;margin-bottom:12px;color:var(--bm-primary);">Nowa pozycja sprzętu</div>
          <div class="bm-alert bm-alert--error" x-show="error" x-text="error"></div>
          <div class="bm-grid-2">
            <div class="bm-field">
              <label class="bm-label">Nazwa *</label>
              <input class="bm-input" x-model="form.name" placeholder="np. Namiot 6-os.">
            </div>
            <div class="bm-field">
              <label class="bm-label">Typ / kategoria</label>
              <input class="bm-input" x-model="form.equipment_type" placeholder="np. camping">
            </div>
            <div class="bm-field">
              <label class="bm-label">Ilość wydana *</label>
              <input class="bm-input" type="number" min="1" x-model.number="form.issued_qty">
            </div>
            <div class="bm-field">
              <label class="bm-label">Uwagi</label>
              <input class="bm-input" x-model="form.notes" placeholder="opcjonalnie">
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button class="bm-btn bm-btn--primary" @click="submit()" :disabled="saving">
            ...
            <button class="bm-btn bm-btn--ghost" @click="showForm=false; error=''">Anuluj</button>
          </div>
        </div>
      </template>

      <div class="bm-empty" x-show="!loading && !showForm && items.length === 0">{{ content.empty_text }}</div>

      <template x-if="!loading && items.length > 0">
        <div class="bm-table-wrap">
          <table class="bm-table">
            <thead>
              <tr><th>Nazwa</th><th>Typ</th><th>Wydano</th><th>Zwrócono</th><th>Do zwrotu</th><th></th></tr>
            </thead>
            <tbody>
              <template x-for="item in items" :key="item.id">
                <tr>
                  <td>
                    ...
                    <div x-show="item.notes" class="bm-meta" x-text="item.notes"></div>
                  </td>
                  <td><span class="bm-badge bm-badge--gray" x-text="item.equipment_type || '—'"></span></td>
                  <td style="text-align:center;font-weight:700;" x-text="item.issued_qty"></td>
                  <td style="text-align:center;" x-text="item.returned_qty"></td>
                  <td style="text-align:center;"><span class="bm-badge" :class="item.outstanding > 0 ? 'bm-badge--gold' : 'bm-badge--green'" x-text="item.outstanding"></span></td>
                  <td><button x-show="item.outstanding > 0" class="bm-btn bm-btn--ghost bm-btn--sm" @click="registerReturn(item.id)">↩ Zwrot</button></td>
                </tr>
              </template>
            </tbody>
          </table>
...
- Alpine: `window.bmEquipment()` z `assets/js/bm-components-social.js`.
- REST / helper: `bmApi.getEquipment()`, `bmApi.issueEquipment(payload)`, `bmApi.returnEquipment(id, qty, nonce)` w `assets/js/bm-api.js`.
- PHP / konfiguracja: zapis i zwroty używają `bmConfig.panelNonce`; komponent korzysta z natywnego `prompt()` do wpisania ilości zwrotu.
...
- Jeśli chcesz zastąpić `prompt()` modalelem Breakdance, wymaga to zmiany `registerReturn()` w źródłowym JS.
- Badge „szt. do zwrotu” liczone jest getterem `totalOutstanding`, więc działa natychmiast po przeładowaniu listy.
