# 06. Formularz zgłoszenia ogłoszenia

Samodzielny formularz do zgłaszania nowego ogłoszenia do akceptacji. Element odpowiada drugiej karcie zakładki „Ogłoszenia”.

## 1. Nazwa elementu w Breakdance
`BM 06 / Formularz zgłoszenia ogłoszenia`

## 2. Kontrolki Element Studio

| Zakładka | Typ | ID | Domyślna wartość | Zastosowanie |
|---|---|---|---|---|
| Content | Text | `content.heading` | `✏ Zgłoś ogłoszenie` | Nagłówek formularza. |
| Content | Text | `content.submit_label` | `Wyślij do zatwierdzenia` | Etykieta głównego przycisku. |
| Content | Textarea | `content.lead` | `Wypełnij pola i wyślij ogłoszenie do zatwierdzenia przez administratora.` | Opcjonalny opis pod nagłówkiem formularza. |
...
```twig
<div class="bm-el" x-data="bmAnnForm()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    ...
    <form class="bm-card-body" @submit.prevent="submit()">
      <p class="bm-meta" style="margin-bottom:14px;">{{ content.lead }}</p>

      <div class="bm-grid-2">
        <div class="bm-field">
          <label class="bm-label">Tytuł *</label>
          <input type="text" class="bm-input" x-model="title" required placeholder="Krótki tytuł">
        </div>
        <div class="bm-field">
          <label class="bm-label">URL załącznika</label>
          <input type="url" class="bm-input" x-model="attachment_url" placeholder="https://…">
        </div>
      </div>

      <div class="bm-field">
        <label class="bm-label">Treść</label>
        <textarea class="bm-textarea" x-model="content" rows="3" placeholder="Treść ogłoszenia…"></textarea>
      </div>

      <div class="bm-grid-2">
        <div class="bm-field">
          <label class="bm-label">Ważne od *</label>
          <input type="date" class="bm-input" x-model="valid_from" required>
        </div>
        <div class="bm-field">
          <label class="bm-label">Ważne do *</label>
          <input type="date" class="bm-input" x-model="valid_until" required>
        </div>
      </div>
      ...
      <button type="submit" class="bm-btn bm-btn--primary bm-btn--block" :disabled="loading">
        <span x-show="loading" class="bm-spinner"></span>
        <span x-text="loading ? 'Wysyłanie…' : '{{ content.submit_label }}'"></span>
      </button>
    </form>
  </div>
</div>
...
- Alpine: `window.bmAnnForm()` z `assets/js/bm-components-auth.js`.
- REST / helper: `bmApi.submitAnnouncement(payload)` w `assets/js/bm-api.js`.
- Store: po sukcesie wywołuje `Alpine.store('bm').loadAnnouncements()` i odświeża wspólną tablicę ogłoszeń.
- Zdarzenia przeglądarki: `bm:annSubmitted`.
- PHP / konfiguracja: wymaga `bmConfig.panelNonce` dla `POST panel/announcements`.
...
- Formularz sam czyści pola po sukcesie; nie trzeba dodawać własnego resetu w Twig.
- Jeśli chcesz ukrywać formularz po wysłaniu, zrób to przez zewnętrzny wrapper w Breakdance, nie przez modyfikację komponentu.
