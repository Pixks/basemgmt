...
Niezależny element osi czasu dnia obozowego z przełączaniem dat, wyróżnieniem zmian i komunikatem o pustym dniu.
...
|---|---|---|---|---|
| Content | Text | `content.heading` | `🗓 Plan dnia` | Nagłówek karty planu dnia. |
| Content | Textarea | `content.empty_text` | `Brak planu na wybrany dzień.` | Komunikat przy pustej liście planów. |
| Content | Text | `content.loader_text` | `Ładowanie planu…` | Tekst loadera. |
...
```twig
<div class="bm-el" x-data="bmSchedule()" x-init="init()" x-cloak x-show="$store.bm.authenticated">
  <div class="bm-card">
    <div class="bm-card-header"><h3>{{ content.heading }}</h3></div>

    <div class="bm-card-body">
      <div class="bm-toolbar">
        <input type="date" class="bm-input" x-model="selectedDate" @change="loadSchedule()" style="max-width:180px;">
        <div class="bm-pills">
      ...
      <div class="bm-loader" x-show="loading">
        ...
        <span>{{ content.loader_text }}</span>
      </div>
      <div class="bm-alert bm-alert--error" x-show="error" x-text="error"></div>
      <div class="bm-empty" x-show="!loading && !plans.length">{{ content.empty_text }}</div>

      <template x-for="plan in plans" :key="plan.id">
        <div style="margin-bottom:14px;">
          <h4 x-show="plan.title" x-text="plan.title" style="margin:0 0 10px;color:var(--bm-primary);font-size:.85rem;text-transform:uppercase;letter-spacing:.04em;"></h4>
          <div class="bm-timeline">
            <template x-for="item in plan.items" :key="item.id">
              <div class="bm-timeline-item" :style="item.item_status==='cancelled' ? 'opacity:.55;' : ''">
                <div style="font-size:.8rem;font-weight:700;color:var(--bm-primary);margin-bottom:3px;" x-text="item.time_from + (item.time_to ? ' – ' + item.time_to : '')"></div>
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                  <span :style="item.item_status==='cancelled' ? 'text-decoration:line-through;' : 'font-weight:700;'" x-text="item.title"></span>
                  ...
                  <span x-show="item.is_mandatory" class="bm-badge bm-badge--blue">obowiązkowe</span>
                  <span x-show="item.item_status==='cancelled'" class="bm-badge bm-badge--red">odwołane</span>
                </div>
                <p x-show="item.description" x-text="item.description" style="margin:3px 0 0;color:#4B5A67;"></p>
                <p x-show="item.location" x-text="'📍 ' + item.location" class="bm-meta" style="margin:2px 0 0;"></p>
              </div>
            </template>
          </div>
        </div>
...
- Alpine: `window.bmSchedule()` z `assets/js/bm-components-content.js`.
- REST / helper: `bmApi.getSchedule(date)`, `bmApi.getScheduleDates()` w `assets/js/bm-api.js`.
- PHP / konfiguracja: backend powinien zwracać `plans` oraz `dates`; komponent odpala oba requesty równolegle w `init()`.
...
- Element jest wierny układowi z panelu: ręczny wybór daty i szybkie „chipsy” dla dni dostępnych z API.
- Flagi `is_new_today`, `is_updated_today`, `is_mandatory` i `item_status` są używane bezpośrednio z payloadu i nie wymagają dodatkowej normalizacji.
